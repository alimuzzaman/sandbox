...
explicit say-so for THAT action (per AGENTS.md). Stage the work, show the diff,
...
(`.Codex/settings.local.json`, stray `*.png` in root, anything under
`runtime/` that isn't gitignored). Then, after the user OKs:

```bash
git add <intended files>
git commit -m "<message>"   # no AI/Codex mentions
