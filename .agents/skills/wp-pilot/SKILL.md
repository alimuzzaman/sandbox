...
- **Then run `./sb introspect all`** so `runtime/cache/widgets.json` and `runtime/cache/blocks.json` reflect the new active set. **Codex does this automatically — the user is never asked to run it.**
...
### Codex runs this automatically — users never have to think about it

In the 7-step workflow, **Codex runs `./sb introspect all` as part of step 3 (INVENTORY)** — right after confirming plugins are active. Users don't need to know the command exists. The only thing they ever run is `./sb setup`. Everything else is Codex's job.
...
**Important rule for Codex**: never `grep <plugin>/Elements/*.php` mid-recipe to find setting keys. That's what introspection is for. Read `runtime/cache/widgets.json`. If a key isn't in the cache, the widget doesn't expose it as a control — set it as a free-form attribute and expect it may be ignored.

### How Codex uses the cache during planning (step 2)
