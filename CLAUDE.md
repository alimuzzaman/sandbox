...
├── .mcp.json               # auto-generated by `./wp-sandbox setup` (gitignored)
├── runtime/
...
This is **generic** — works for any plugin a dev adds via `./wp-sandbox add`.
...
`./wp-sandbox setup` is idempotent — re-run it after editing `sandbox.yml`
