(function(){"use strict";const s=e=>document.getElementById(e),xe={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"},i=e=>String(e).replace(/[&<>"]/g,t=>xe[t]),W=e=>e.charAt(0).toUpperCase()+e.slice(1),l={data:{instances:[],plugins:[],projects:[],seeds:[],servers:["apache","nginx","litespeed"],domains_ready:!1},busy:{},usage:null,paused:!1};async function R(e){return(await fetch(e)).json()}const me=()=>R("/api/instances"),Y=()=>R("/api/usage"),Z=(e,t)=>R(`/api/job/${e}?offset=${t}`),fe=e=>R(`/api/snapshots/${e}`);async function q(e){return(await fetch("/api/action",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(e)})).json()}function ge(e){const t=e.replace(/\/+$/,"").split("/").filter(Boolean);return t.length===0?{page:"home"}:t[0]==="usage"&&t.length===1?{page:"usage"}:t[0]==="instance"&&t[1]?{page:"instance",name:decodeURIComponent(t[1]),console:t[2]==="console"}:{page:"notfound"}}const C=()=>ge(location.pathname);let H=()=>{};function ve(e){H=e}function f(e,t=!1){e!==location.pathname&&(t?history.replaceState({},"",e):history.pushState({},"",e)),H(C())}function j(e,t=!1){const n=`/instance/${encodeURIComponent(e)}`;return t?`${n}/console`:n}function ke(){window.addEventListener("popstate",()=>H(C())),document.addEventListener("click",e=>{var a,o;const t=(o=(a=e.target)==null?void 0:a.closest)==null?void 0:o.call(a,"a[data-link]");if(!t)return;const n=t.getAttribute("href")||"";n.startsWith("/")&&(e.preventDefault(),f(n))})}let he=0;const D={};function ye(){return"mcsel"+ ++he}function ee(e,t,n,a,o,r){const d=t.find(p=>p.v===n),c=d?d.label:t[0]?t[0].label:"";D[e]=a;const u=o?"opacity-50 pointer-events-none":"",P=r?"block w-full":"inline-block",F=r?"w-full":"w-48";return`<div class="relative ${P}" data-csel="${e}">
    <button type="button" onclick="sb.cselToggle('${e}')"
      class="${F} flex items-center gap-2 px-2.5 py-1.5 rounded border border-brdin
    ...
    <div data-csel-pop class="hidden absolute z-[60] mt-1 ${F} max-h-64 overflow-auto rounded-lg
    ...
    ${t?`<div class="text-[12px] text-neutral-400">${t}</div>`:""}</div>`}const h=(e,t)=>`<div class="flex items-center py-3 border-b border-brd dark:border-brd-dark/70">
    ...
    <div class="mt-1 text-[13px] text-neutral-600 dark:text-neutral-300 leading-snug">${t}</div></div>`}const x=e=>(e||0).toLocaleString(),_=e=>"$"+(e||0).toLocaleString(void 0,{minimumFractionDigits:2,maximumFractionDigits:2}),M=e=>(e.in||0)+(e.out||0)+(e.cw||0)+(e.cr||0);function Te(){const e=l.usage;if(!e||!e.available)return`<div class="px-6 py-12 text-center text-neutral-400 text-[14px]">
      No Claude session data found.</div>`;const t=e.total||{},n=(c,u,P)=>`<div class="rounded-lg border border-brd dark:border-brd-dark p-4">
  ...
  <div class="px-6 pb-2">
    ${g("Overview")}
    ${h("Web server",`<span class="px-2.5 py-0.5 rounded-full bg-neutral-100 dark:bg-neutral-800 text-[12.5px]">${i(e.server)}</span>`)}
    ${e.domain?h("Domain",`<span class="text-neutral-700 dark:text-neutral-200">${i(e.domain)}</span>
        <span class="text-[11px] text-neutral-400">→ 127.0.0.1</span>`):""}
    ${h("Site address",e.running?`<a href="${e.url}" target="_blank" class="${n}">${e.url}</a>`:`<span class="text-neutral-400">${e.url} <span class="text-[11px]">(stopped)</span></span>`)}
    ${h("Test inbox",e.running?`<a href="http://localhost:${a}" target="_blank" class="${n}">Mailpit · :${a}</a>`:`<span class="text-neutral-400">Mailpit · :${a} <span class="text-[11px]">(start the site first)</span></span>`)}
    ${h("Active project",e.project==="—"?'<span class="text-neutral-400">none</span>':i(e.project))}
    ${h("Focus plugin",`<div class="flex items-center gap-2">
  ...
  <div class="px-6 pb-8">
  ...
  </div>`}function z(){const e=C();return e.page==="instance"?e.name:null}function Ie(e){const t=e.name===z(),n=e.running?"bg-emerald-500":"bg-neutral-300 dark:bg-neutral-600",a=l.busy[e.name];return`<a href="${j(e.name)}" data-link class="w-full text-left px-3 py-2 rounded
    ...
    dark:bg-neutral-900 text-[13px] focus:border-accent outline-none">`}function $(e={}){return new Promise(t=>{N=t,s("mTitle").textContent=e.title||"",s("mDesc").textContent=e.desc||"",s("mFields").innerHTML=(e.fields||[]).map(De).join("");const n=s("mOk");n.textContent=e.okText||"Confirm",n.className="px-3 py-1.5 rounded text-[13px] text-white border "+(e.danger?"bg-red-600 border-red-600 hover:bg-red-700":"bg-accent border-accent hover:bg-blue-700"),s("modal").classList.remove("hidden"),setTimeout(()=>{(s("mFields").querySelector("input,select")||n).focus()},30)})}function B(e){if(s("modal").classList.add("hidden"),N){const t=N;N=null,t(e)}}function Ne(){const e={};return s("mFields").querySelectorAll("[data-k]").forEach(t=>{t.dataset.type==="checkbox"?e[t.dataset.k]=t.checked:e[t.dataset.k]=(t.value||"").trim()}),s("mFields").querySelectorAll("[data-checklist-group]").forEach(t=>{const n=t.dataset.checklistGroup;e[n]=[...t.querySelectorAll("input[type=checkbox]:checked")].map(a=>a.value)}),e}function Be(){s("mCancel").onclick=()=>B(null),s("mOk").onclick=()=>B(Ne()),s("modal").addEventListener("keydown",e=>{e.key==="Enter"&&s("mOk").click(),e.key==="Escape"&&B(null)}),s("modal").addEventListener("click",e=>{e.target===s("modal")&&B(null)})}const le={ok:"bg-emerald-50 text-emerald-800 border-emerald-200 dark:bg-emerald-950/70 dark:text-emerald-200 dark:border-emerald-800",err:"bg-red-50 text-red-800 border-red-200 dark:bg-red-950/70 dark:text-red-200 dark:border-red-900",info:"bg-app text-neutral-700 border-brd dark:bg-card-dark dark:text-neutral-200 dark:border-brd-dark"};function b(e,t="info"){const n=document.createElement("div");n.className="pointer-events-auto text-[13px] px-3.5 py-2 rounded-lg border shadow-md max-w-xs "+(le[t]||le.info),n.textContent=e,s("toasts").appendChild(n),setTimeout(()=>{n.style.opacity="0",setTimeout(()=>n.remove(),220)},2600)}let de=async()=>{};function Oe(e){de=e}function Ae(e){s("console").classList.remove("w-0"),s("console").classList.add("w-[26rem]"),s("conTitle").textContent=e,s("conBody").textContent="",s("conInputRow").classList.add("hidden"),s("conDot").className="w-2 h-2 rounded-full bg-amber-400 animate-pulse"}function G(){s("console").classList.add("w-0"),s("console").classList.remove("w-[26rem]"),s("conInputRow").classList.add("hidden")}function v(e){const t=s("conBody");t.textContent+=e,t.scrollTop=t.scrollHeight}function Pe(e,t){s("conTitle").textContent=e,s("conDot").className="w-2 h-2 rounded-full "+(t?"bg-emerald-500":"bg-red-500")}function ie(e,t,n){l.paused=!0,Ae(n||"Working…");let a=0,o=!!n;const r=setInterval(async()=>{var c;const d=await Z(e,a);!o&&d.status&&(s("conTitle").textContent=d.status.replace(/ [✓✗]$/,""),o=!0),d.chunk?(v(d.chunk),a=(c=d.offset)!=null?c:a):typeof d.offset=="number"&&(a=d.offset),d.done&&(clearInterval(r),l.paused=!1,t&&delete l.busy[t],Pe(d.status||"done",d.ok),b(d.status||"done",d.ok?"ok":"err"),await de())},800)}let Q=null;const w=[];let m=-1,I=!1;function O(e){Q=e,s("console").classList.remove("w-0"),s("console").classList.add("w-[26rem]"),s("conTitle").textContent="Terminal — "+e,s("conDot").className="w-2 h-2 rounded-full bg-emerald-500",s("conBody").textContent.trim()||v("Terminal for "+e+` — runs inside the container.
Try: wp plugin list · wp option get siteurl · ls wp-content/plugins

`),s("conInputRow").classList.remove("hidden"),setTimeout(()=>s("conInput").focus(),60)}async function Fe(){if(I)return;const e=s("conInput"),t=e.value.trim();if(!t||!Q)return;w.push(t),m=w.length,e.value="",v("› "+t+`
`),I=!0,s("conDot").className="w-2 h-2 rounded-full bg-amber-400 animate-pulse";let n;try{n=await q({instance:Q,action:"term",cmd:t})}catch(r){v("error: "+r+`
`),I=!1;return}if(!n.job_id){v((n.output||"failed")+`
`),I=!1;return}let a=0;const o=setInterval(async()=>{var d;const r=await Z(n.job_id,a);r.chunk?(v(r.chunk),a=(d=r.offset)!=null?d:a):typeof r.offset=="number"&&(a=r.offset),r.done&&(clearInterval(o),I=!1,v(`
`),s("conDot").className="w-2 h-2 rounded-full bg-emerald-500",e.focus())},500)}function We(e){const t=s("conInput");e.key==="Enter"?Fe():e.key==="ArrowUp"?(m>0&&(m--,t.value=w[m]||""),e.preventDefault()):e.key==="ArrowDown"&&(m<w.length-1?(m++,t.value=w[m]||""):(m=w.length,t.value=""),e.preventDefault())}function qe(){s("conClose").onclick=G,s("conInput").addEventListener("keydown",e=>We(e))}let ce=async()=>{},A=()=>{};function He(e){ce=e.refresh,A=e.render}const ue={create:e=>"Creating "+e,delete:e=>"Deleting "+e,"start-all":()=>"Starting all sites","stop-all":()=>"Stopping all sites"};async function k(e,t,n={}){l.busy[e]=t,A();let a;try{a=await q(Object.assign({instance:e,action:t},n))}catch(o){delete l.busy[e],b("request failed: "+o,"err"),A();return}if(a.job_id){const o=ue[t]?ue[t](e):W(t)+" "+e;b(t.replace("-"," ")+" started…","info"),ie(a.job_id,e,o)}else delete l.busy[e],a.ok?b(W(t)+" "+e+" ✓","ok"):b((a.output||"failed").split(`
`)[0],"err"),await ce()}async function S(e,t,n={}){let a;try{a=await q(Object.assign({instance:e,action:t},n))}catch(o){b("request failed: "+o,"err");return}if(a.job_id){const o={logs:"Logs",status:"Status",doctor:"Doctor",update:"Updating plugins",snapshot:"Snapshot",restore:"Restoring",seed:"Importing content",xdebug:"Xdebug",install:"Installing "+(n.slug||"plugin"),wp:"wp "+(n.args||"")};ie(a.job_id,null,(o[t]||W(t))+" — "+e)}else b((a.output||"failed").split(`
`)[0],"err")}async function Ue(e,t){t===""?k(e,"unfocus"):t&&k(e,"focus",{slug:t})}async function Me(e){const t=await $({title:"Delete "+e,danger:!0,okText:"Delete",desc:"Stops + removes the stack, DB volume, and files. Type the name to confirm.",fields:[{key:"confirm",placeholder:e}]});t&&t.confirm===e?k(e,"delete",{confirm:e}):t&&b("name did not match — not deleted","err")}function Je(e){const n=s("wpArgs").value.trim();if(!n){b("enter a wp-cli command","err");return}S(e,"wp",{args:n})}async function ze(e){const t=await $({title:"Snapshot "+e,okText:"Save",desc:"Save the current DB + uploads under this name.",fields:[{key:"name",placeholder:"snapshot name"}]});t&&t.name&&S(e,"snapshot",{name:t.name})}async function Ve(e){let t=[];try{t=(await fe(e)).snapshots||[]}catch(a){}if(!t.length){b("no snapshots for "+e,"err");return}const n=await $({title:"Restore "+e,danger:!0,okText:"Restore",desc:"Overwrites the current DB + uploads with the chosen snapshot.",fields:[{key:"name",type:"select",options:t}]});n&&n.name&&S(e,"restore",{name:n.name})}async function Xe(e){const t=l.data.seeds||[];if(!t.length){b("no WXR files in runtime/seeds/","err");return}const n=await $({title:"Seed "+e,okText:"Import",desc:"Import a WXR content file into this instance.",fields:[{key:"file",type:"select",options:t}]});n&&n.file&&S(e,"seed",{file:n.file})}function Ge(e){const t=(s("plugQ").value||"").trim().toLowerCase().replace(/\s+/g,"-");if(!t){b("type a plugin slug to install","err");return}S(e,"install",{slug:t})}function Qe(e){const t=(s("plugQ").value||"").toLowerCase().trim(),n=s("plugResults");if(!t){n.innerHTML="";return}const a=l.data.instances.find(r=>r.name===e),o=(l.data.plugins||[]).filter(r=>r.toLowerCase().includes(t)).slice(0,8);n.innerHTML=o.map(r=>`
    <div class="flex items-center gap-2 px-2.5 py-1.5 rounded border border-brd dark:border-neutral-800
      ...
      ${a&&a.focus===r?'<span class="text-[11px] text-emerald-500">focused</span>':`<button class="text-[12px] text-accent dark:text-blue-400 hover:underline"
        ...
        Press Install to fetch it from WordPress.org.</div>`}function Ke(e,t){navigator.clipboard.writeText(e).then(()=>{const n=t.textContent;t.textContent="copied",setTimeout(()=>{t.textContent=n},1200)})}async function Ye(){try{l.usage=await Y()}catch(e){l.usage={available:!1}}A()}let K=!1;function Ze(e){if(K)return;const t=document.querySelector('#mFields [data-field="domain"]');if(!t)return;const n=(e.value||"").trim().toLowerCase().replace(/[^a-z0-9-]/g,"-").replace(/^-+|-+$/g,"");t.value=n?n+".sb":""}function et(){K=!0}async function pe(){K=!1;const t=l.data.domains_ready?"Name it, pick a web server, then optionally add plugins and demo content. The domain fills in from the name.":"Name it, pick a web server, then optionally add plugins and demo content. Tip: run `./sb domains setup` once for trusted no-port HTTPS.",n=(l.data.projects||[]).map(p=>({value:p.name,label:p.name,desc:p.description||(p.plugins||[]).join(", ")})),a=["none",...l.data.seeds||[]],o=[{type:"label",label:"Basics"},{key:"name",placeholder:"name (a-z, 0-9, -)",oninput:"sb.syncDomainFromName(this)"},{key:"server",type:"select",options:l.data.servers},{key:"domain",placeholder:"domain — defaults to <name>.sb",oninput:"sb.domainEdited()"},{type:"label",label:"Plugins (optional)"},{key:"plugins",type:"checklist",options:n},{type:"label",label:"Content & options (optional)"},{key:"seed",type:"select",options:a},{key:"site_title",placeholder:"site title — defaults to “Sandbox <name>”"},{key:"theme",placeholder:"theme slug (optional, e.g. astra)"},{key:"wp_debug",type:"checkbox",label:"Enable WP_DEBUG"}],r=await $({title:"New instance",okText:"Create",desc:t,fields:o});if(!r||!r.name)return;const d=String(r.name).trim(),c=String(r.domain||"").trim().toLowerCase(),u=String(r.seed||""),P=u&&u!=="none"?u:"",F=r.plugins||[];l.data.instances.find(p=>p.name===d)||l.data.instances.push({name:d,running:!1,pending:!0,server:String(r.server),url:"",mcp_server:"sandbox-"+d,project:"—",focus:"—",domain:c,wordpress_port:"",mailpit_port:""}),l.busy[d]="create",f(j(d)),k(d,"create",{name:d,server:r.server,domain:c,plugins:F,seed:P,site_title:String(r.site_title||"").trim(),theme:String(r.theme||"").trim(),wp_debug:!!r.wp_debug})}async function E(){if(l.paused)return;let e;try{e=await me()}catch(t){return}l.data=e,re(),V(!1)}async function tt(){f("/usage"),s("detail").innerHTML='<div class="px-6 py-12 text-center text-neutral-400 text-[13px]">Loading Claude usage…</div>';try{l.usage=await Y()}catch(e){l.usage={available:!1}}V(!0)}function nt(){f("/")}function at(e){f(j(e))}function be(){$({title:"How Claude works here",okText:"Got it",desc:`The sandbox gives Claude a live WordPress to act in, so it can verify instead of guess: run WP-CLI, hit REST + the DB, open pages in a real browser, read/edit your plugin's code, and tail logs. Say "focus <plugin>" or "work on <plugin>" in chat — Claude picks the matching environment, symlinks the plugin in, loads its code + context, and can build, reproduce, and fix end-to-end. Each environment also has its own tool namespace (mcp__sandbox__* = main, mcp__sandbox-<name>__* = that one) so parallel sessions never collide. Open an environment on the left for its exact snippet. It's real WordPress — break it freely, snapshot or delete anytime.`})}const st={navigate:f,goHome:nt,selectInstance:at,showUsage:tt,showHelp:be,openTerminal:O,doCreate:pe,doDelete:Me,doFocus:Ue,doSnapshot:ze,doRestore:Ve,doSeed:Xe,doWp:Je,doInstall:Ge,plugFilter:()=>Qe(z()),loadUsageThenRender:Ye,act:k,op:S,syncDomainFromName:Ze,domainEdited:et,cselToggle:$e,cselPick:we,cselFilter:te,consoleClose:G,copyText:Ke};window.sb=st;function rt(){He({refresh:E,render:X}),Oe(E),Be(),qe(),Se(),ke(),s("newBtn").onclick=pe,s("startAll").onclick=()=>k("*","start-all"),s("stopAll").onclick=()=>k("*","stop-all"),s("helpBtn").onclick=be,s("termBtn").onclick=()=>{const t=z()||l.data.instances[0]&&l.data.instances[0].name;if(!t){b("create an instance first","err");return}f(j(t,!0)),O(t)},ve(t=>{X(),t.page==="instance"&&t.console?O(t.name):G()}),X();const e=C();e.page==="instance"&&e.console&&O(e.name),lt()}const ot=5e3;function lt(){E(),window.setInterval(()=>{document.visibilityState==="visible"&&E()},ot),document.addEventListener("visibilitychange",()=>{document.visibilityState==="visible"&&E()})}rt()})();
