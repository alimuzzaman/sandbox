...
- the local durable workspace-index migration is implemented and regression-tested;
  remote rollout still requires an exact read-only plan, zero collisions, unchanged
  digest/generation, and before/after job/network counts. Legacy metadata remains
  byte-preserved, and unresolved records fail with `workspace_index_incomplete`;
