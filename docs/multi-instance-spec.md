...
| **004, 005, 006, 007, 011, 012** | async jobs, editor authoring, skill authoring, debugging tools, EB schema, bundled schema catalog | **Unaffected.** Jobs/logs/xdebug/qm are per instance-name; schema catalog + editor schema are source/global reads; skill tools read the shared source repo. Their MCP tools gain the routing-only `label` param but no FR changes. |
