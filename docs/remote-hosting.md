...
| `./sb wp --remote <name> --project-dir <dir> --timeout <seconds> -- <wp args...>` | Run bounded WP-CLI against that project's existing deployed WordPress instance through authenticated control HTTP; request/unit/live runtime digests and stable non-symlink deploy identity must match; timeout or output overflow is nonzero unknown; no workspace creation, generic exec, SSH fallback, or automatic retry |
