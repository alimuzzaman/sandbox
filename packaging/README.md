...
| npm | `@wpdeveloper/sandbox` (`package.json` + `bin/sandbox.js`) | Node devs / CI |
...
npm install -g ./wpdeveloper-sandbox-*.tgz # or: npm i -g @wpdeveloper/sandbox
sandbox --help
```
...
(`WPDeveloper/homebrew-sandbox` as `Formula/sandbox.rb`). It depends on
...
```bash
brew install --HEAD wpdeveloper/sandbox/sandbox   # from git
# or, after a release is published + the formula sha is filled:
brew tap wpdeveloper/sandbox && brew install sandbox
```
