...
def _parse_output_wait_argument(value: str) -> object:
    """Keep malformed CLI input available for the command's JSON error path.

    ``argparse(type=int)`` exits before ``cmd_job_output`` can emit the stable
    ``invalid_output_query`` envelope. Valid decimal whole seconds are decoded
    here; everything else is passed through to the shared normalizer.
    """
...
def cmd_job_output(_cfg, args) -> None:
    ...
    try:
        ...
        if getattr(args, "json", False):
            _emit_json_line({"ok": False, "code": "invalid_output_query", "error": str(exc)})
            raise SystemExit(1)
        _die(str(exc))
    ...
    if args.remote:
        ...
        while True:
            ...
            # Health classification may inspect a remote process identity. Keep
            # follow bounded even when that inspection stalls; callers can
            # resume with a normal job-status request instead of having Ctrl-C
            # interrupt the output command.
            try:
                ...
                unknown = exc.to_payload(
                    remote=args.remote, operation="job-status",
                )
                unknown.update({
                    "job_id": args.job_id,
                    "follow": "stopped",
                ...
                if args.json:
                    ...
                    print(
                        "warning: remote job status became unavailable; "
                        "follow stopped. Retry job-status or job-output without "
                        "--follow.",
                        file=sys.stderr,
                    )
                return
