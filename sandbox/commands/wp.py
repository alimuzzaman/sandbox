...
def _stage_host_package_paths(
    ...
    """Expose supported host file paths through the Docker mount.

    ``sb wp`` executes inside the selected container, so a host ZIP, media
    fixture, or eval-file script otherwise looks like a path that exists only
    on the host. The existing shared download-cache mount is the narrowest
    transfer boundary; stage regular files there under a private temporary name
    and remove them after the call. Herd keeps using the host path directly.
    """
    supported = {
        ("plugin", "install"),
        ("theme", "install"),
    ...
    command = tuple(argv[:2]) if argv[:2] in (["plugin", "install"],
                                               ["theme", "install"],
                                               ["media", "import"]) else tuple(argv[:1])
