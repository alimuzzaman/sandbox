...
def _drive_restore_command(paths: dict, destination: str, backup_id: str) -> str:
    ...
    return (
        ...
        "gateway_was_active=0; dashboard_was_active=0; "
        "systemctl --user is-active --quiet hermes-gateway-sandbox.service 2>/dev/null && gateway_was_active=1 || true; "
        "systemctl --user is-active --quiet hermes-dashboard-sandbox.service 2>/dev/null && dashboard_was_active=1 || true; "
        f"previous_home=\"$HOME/.hermes.pre-restore\"; previous_sandbox={home}.pre-restore; previous_gh=\"$HOME/.config/gh.pre-restore\"; previous_rclone=\"$HOME/.config/rclone.pre-restore\"; "
        "had_home=0; had_sandbox=0; had_gh=0; had_rclone=0; "
        "test ! -e \"$HOME/.hermes\" || had_home=1; test ! -e " + home + " || had_sandbox=1; "
        "test ! -e \"$HOME/.config/gh\" || had_gh=1; test ! -e \"$HOME/.config/rclone\" || had_rclone=1; "
        "rm -rf \"$previous_home\" \"$previous_sandbox\" \"$previous_gh\" \"$previous_rclone\"; "
        "restore_path() { current=\"$1\"; previous=\"$2\"; had=\"$3\"; if test \"$had\" = 1; then if test -e \"$previous\"; then rm -rf \"$current\"; mv \"$previous\" \"$current\"; fi; else rm -rf \"$current\"; fi; }; "
        "committed=0; rollback() { rc=$?; if test \"$committed\" = 0; then set +e; restore_path \"$HOME/.hermes\" \"$previous_home\" \"$had_home\"; restore_path " + home + " \"$previous_sandbox\" \"$had_sandbox\"; restore_path \"$HOME/.config/gh\" \"$previous_gh\" \"$had_gh\"; restore_path \"$HOME/.config/rclone\" \"$previous_rclone\" \"$had_rclone\"; if test \"$gateway_was_active\" = 1; then systemctl --user start hermes-gateway-sandbox.service || true; fi; if test \"$dashboard_was_active\" = 1; then systemctl --user start hermes-dashboard-sandbox.service || true; fi; fi; rm -rf \"$stage\"; exit \"$rc\"; }; trap rollback EXIT; "
        ...
        "committed=1; systemctl --user daemon-reload 2>/dev/null || true; systemctl --user start hermes-gateway-sandbox.service 2>/dev/null || true; systemctl --user start hermes-dashboard-sandbox.service 2>/dev/null || true; "
        "printf '%s\\n' restored"
    )
