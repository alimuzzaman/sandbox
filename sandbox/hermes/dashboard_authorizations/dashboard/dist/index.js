(function () {
  ...
  function Page() {
    ...
    useEffect(() => { refresh(); }, []);
    const sync = () => { setBusy(true); request("/sync", {method: "POST"}).then(refresh).catch(e => setError(errorMessage(e))).finally(() => setBusy(false)); };
    const approve = id => { if (!window.confirm("Approve this exact scoped request?")) return; setBusy(true); request("/requests/" + id + "/approve", {method: "POST", body: JSON.stringify({confirm: true})}).then(refresh).catch(e => setError(errorMessage(e))).finally(() => setBusy(false)); };
    const create = event => { event.preventDefault(); setBusy(true); request("/requests", {method: "POST", body: JSON.stringify({...form, expires_in_minutes: Number(form.expires_in_minutes)})}).then(refresh).catch(e => setError(errorMessage(e))).finally(() => setBusy(false)); };
