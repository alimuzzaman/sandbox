...
def memory_high_bytes(memory_bytes):
    """Return the fixed reclaim threshold paired with a hard memory ceiling."""
    return max(1, memory_bytes * 9 // 10)
...
class ResourcePolicyCompiler:
    def compile(self, values):
        ...
        ranges = {"cpu_percent": (10, 6400), "memory_bytes": (128 * 1024**2, 256 * 1024**3),
                  ...
                  "fds": (128, 1048576), "connections": (16, 20000), "io_weight": (1, 10000)}
        ...
        return {**policy, "systemd": {
            ...
            "MemoryHigh": memory_high_bytes(policy["memory_bytes"]),
            "MemorySwapMax": 0, "TasksMax": policy["pids"],
            "LimitNOFILE": policy["fds"],
