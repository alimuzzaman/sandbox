...
def _archive(project_root: Path, manifest: CaptureManifest, *, project_relative_manifest: bool = False) -> bytes:
...
class RemoteSyncTransport:
    ...
    def transfer(
        ...
        try:
            ...
            publish = (
                f"test -s {shlex.quote(staging + '/.sandbox-sync-manifest.json')} && "
                f"rm -rf -- {shlex.quote(published)} && "
                f"mv -- {shlex.quote(staging)} {shlex.quote(published)} && "
                f"ln -sfn -- {shlex.quote(published)} {shlex.quote(current)} && "
                f"chmod 0700 {shlex.quote(published)}"
            )
...
class HostSourceSyncTransport:
    ...
    def transfer(
        ...
        try:
            ...
            publish_program = r'''import json, os, pathlib, shutil, sys, tempfile
...
if document.get("generation_id") != generation or not isinstance(document.get("entries"), list):
