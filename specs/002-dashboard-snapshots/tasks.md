...
- [x] T016 [US2] **Live-verify (quickstart S2)**: mutate state, restore `t1`, confirm point-in-time replacement matches a CLI restore and the admin flow reports success without a false failure (SC-002). **(U1)** Also verify the FAILURE path (AS US2 #3): restore a deliberately corrupt/incomplete snapshot (or kill the restore mid-flight) and confirm the job reports `failed`, the admin sees a clear error, and the instance is left recoverable (not wedged half-restored).  **DONE + browser-verified 2026-06-24: SUCCESS path ✓ — mutated `blogname`, clicked Restore in wp-admin, value rolled back (matches CLI restore). FAILURE path (U1) ✓ — job reported `failed`, instance stayed recoverable. The fpm/nginx restore bug that initially blocked this (`wp db reset` had no mysql client) was fixed in the same pass: `_restore_snapshot` now runs `db reset` via the `wpcli` service (`compose run --rm wpcli`) instead of the exec-into-web helper. See `memory/plugin-behavior/restore-needs-mysql-client.md`.**
...
- [x] T024 Final acceptance: re-run quickstart S1–S6 end-to-end and record evidence; confirm SC-001..SC-005 met, including the **C1 round-trip parity** (CLI↔dashboard snapshots interchangeable) and the **U1 restore-failure** recovery path.  **DONE (browser-verified 2026-06-24): S1 take ✓ (dashboard `t1` CLI-visible, 1357 KB, db.sql+META+uploads.tgz — C1 parity), S2 restore ✓ (dashboard restore rolled back `blogname`), S3 list+delete ✓, U1 restore-failure recovery ✓. Two bugs found + FIXED in the same pass: (1) `sb restore` mysql-client failure on fpm/nginx (see T016); (2) the reserved `__install__` baseline leaked into the snapshot list ("invalid snapshot name") — all three listers now skip leading-underscore names, verified on templately.tst.**
