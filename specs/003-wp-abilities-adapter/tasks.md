...
- [~] T022 Driver parity (quickstart §7): on a herd instance via its `.test` endpoint, repeat execute-php + crash-recovery **and** connect + gating + a file-ability round-trip (confirms app-password auth over the herd SSL endpoint), per analysis C3. **Attempted 2026-07-16 on installed Herd 1.29.0:** linking, TLS, PHP 8.3 isolation, and WordPress core download succeeded; the host MySQL endpoint `127.0.0.1:3306` was unavailable, so the database install and remaining acceptance scenarios cannot run until the operator restores/configures host MySQL. No service was started or reconfigured.
