...
- [~] T022 Driver parity (quickstart §7): on a herd instance via its `.test` endpoint, repeat execute-php + crash-recovery **and** connect + gating + a file-ability round-trip (confirms app-password auth over the herd SSL endpoint), per analysis C3. **Updated 2026-08-15:** a disposable Herd 1.29.0 / PHP 8.3 instance provisions against external DBngin. The Herd lifecycle now reconciles the host-compatible abilities and debugging MU plugins on fresh install, `up`, and `apply_config`; live `up` changed the existing instance from `loader:false, ability:false` to `loader:true, ability:true` and also restored the dump/QM loaders. Focused lifecycle tests and an independent review passed. **Remaining live gate:** complete the authenticated external-client execute-php, connect/gating, crash-recovery, and file-ability round-trip over the Herd `.test` endpoint; do not treat local WP-CLI registration evidence as that MCP acceptance.
