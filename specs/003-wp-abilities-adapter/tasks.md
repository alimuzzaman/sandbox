...
- [~] T012 [US2] Local implementation and harness complete: the Sandbox server now exposes `mcp-adapter/discover-abilities`, requires an authenticated `manage_options` user through its server-scoped public transport callback, preserves the adapter's `abilities` list byte-for-byte, and appends a bounded `sandbox_environment` only for a valid successful Sandbox-server discovery result. Unrelated adapter servers and explicit failure/error envelopes remain unchanged. Provision/focus/clear/stolen-focus paths atomically synchronize a host-generated context containing only a validated slug or `null`; URL userinfo/query/fragment data and malformed context are excluded. **Remaining live gate:** from an authenticated `manage_options` external MCP client, call `mcp-adapter-discover-abilities` against a provisioned instance and capture the unchanged list plus exact guidance; also prove an authenticated under-privileged client is refused. Local evidence: `tests/test_spec003_discovery_guidance.py`, PHP lint, interface/docs and extraction-boundary suites.
...
- [~] T022 Driver parity (quickstart §7): on a herd instance via its `.test` endpoint, repeat execute-php + crash-recovery **and** connect + gating + a file-ability round-trip (confirms app-password auth over the herd SSL endpoint), per analysis C3. **Updated 2026-08-14:** a disposable Herd 1.29.0 / PHP 8.3 instance now provisions successfully against external DBngin; WordPress core, REST authentication, and an admin Application Password are live. Parity remains blocked because the provisioned Herd site has no `00-sandbox-abilities.php`: `cmd_up` returns on the Herd branch before the shared host-file mu-plugin writers, and the Herd `apply_config` branch likewise does not write them. The required fix cannot be shipped from this session because the mandatory `fix` workflow's live Sandbox MCP before/after surface is unavailable. The external-client app-password call also remains human-gated by the secret broker.
