...
- [x] T017 Make Query Monitor provisioned installed-but-inactive by default, retaining first-capture activation and updating default-policy coverage, per FR-005 and SC-004 (contradicts). **DONE: resolved-default, inactive provisioning, and first-capture ordering are covered by focused unit tests; the AGENTS example now matches the installed-inactive default. This task adds no fresh live-stack proof or measured overhead evidence.**
