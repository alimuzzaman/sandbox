...
- **FR-002**: The system MUST let a project declare, via its own project configuration, which plugin to check — there is no reasonable default, so a project that has not declared this MUST receive a clear, actionable error rather than run against an arbitrary or guessed plugin.
