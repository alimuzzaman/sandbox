...
- [ ] T040 Propagate cancellation through CLI, MCP, service, and collectors and emit explicit cancelled or disconnected completion while preserving completed evidence per FR-003, FR-023, and the Deep Attribution Request entity (CLI/service/collector and synthetic MCP boolean coverage complete; real MCP lifecycle cancellation/disconnect requires reviewed request context and revision evidence)
...
- [ ] T045 Add deterministic hard-link, multi-filesystem, nested-mount, permission-partial, delivered-partial timeout, repeatability, semantic-parity, isolation, zero-mutation, and live acceptance evidence per SC-001 through SC-011, SC-013, SC-014, and T029 (deterministic coverage present; fresh named-remote live zero-mutation and timing evidence remains required)
