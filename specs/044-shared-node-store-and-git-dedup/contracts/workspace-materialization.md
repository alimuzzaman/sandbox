...
This contract is the normative boundary for the planned
...
behaviour; it does not assert that either implementation or its tests exist.

## Inputs and outputs

### Python seam

The implementation should expose one pure-planning/one-I/O split so both callers use the
same rules:
...
1. Resolve and validate source/workspace ownership and acquire the per-source lock. The lock
   ...
   and cleanup. If held by another operation, return `workspace_materialization_busy` without
   deleting the existing workspace.
...
4. Classify `source/.git`:
   ...
   - **Regular marker file**: parse a bounded `gitdir: <path>` value. When it resolves to a
     ...
     rewrite the workspace marker to that path. Do not hard-link any object. For malformed or
     unreadable markers, perform the complete plain-copy compatibility path and return
     `fallback_reason=malformed_marker` or `source_unreadable`; do not leave a marker pointing
     into the source.
   - **Other type**: select the plain-copy fallback with a bounded reason.

5. Publish the staged `.git` directory atomically after all files are present and fsynced.
...
6. Release the lock only after the receipt is durable. A later refresh may replace the
   workspace contents, but it must acquire the same lock and repeat the atomic publication.
...
- Source deployment/reset and workspace materialization use one source lock key. The remote
  deploy path (`sandbox/core/_remote.py::reset_target_to` and
  `apply_uncommitted`) must hold/coordinate that lock before a workspace snapshot is taken.
