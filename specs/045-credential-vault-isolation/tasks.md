...
- [ ] T035 [US3] Implement the reviewed unprivileged standalone broker executable and its instance-bound request/trusted-lease endpoints in `tools/native-helper/native-credential-broker.py`, then satisfy `tests/test_credential_broker_service_contract.py` without adding a default or adoptable runtime path (LOCAL ONLY: the runnable coordinator is present -- retained guest connection, authenticated controller endpoint, descriptor rendezvous, descriptor-backed typed execution through the existing broker gates, terminal SBRS delivery, bounded worker loop, strict config, and lifecycle identity, all covered by fake-socket tests. The kernel-derived guest connection observer is implemented and fail-closed. T035 remains open for the control-plane credential adapter, which keeps the executable's own `--serve` path a refusal, and for authorized Ubuntu proof under T022/T029/T031; code presence is not T022 proof)
