...
- [ ] T035 [US3] Implement the reviewed unprivileged standalone broker executable and its instance-bound request/trusted-lease endpoints in `tools/native-helper/native-credential-broker.py`, then satisfy `tests/test_credential_broker_service_contract.py` without adding a default or adoptable runtime path (LOCAL ONLY: guarded transport/descriptor seams, the guest request and terminal-result codecs, and pure operation/controller state seams are present; the legacy descriptor registry binds operation/request digests but is not the new operation flow. T035 remains open for one retained guest connection through authenticated claim, descriptor rendezvous, typed upstream execution, terminal delivery, runnable cross-process coordination, and lifecycle observation. Authorized host proof remains under T022/T029/T031; code presence is not T022 proof)
