...
- [ ] T035 [US3] Implement the reviewed unprivileged standalone broker executable and its instance-bound request/trusted-lease endpoints in `tools/native-helper/native-credential-broker.py`, then satisfy `tests/test_credential_broker_service_contract.py` without adding a default or adoptable runtime path (LOCAL ONLY: guarded transport/descriptor seams, retained guest/controller connections, authenticated lifecycle/claim state, internal claim-owner rendezvous, typed execution, terminal SBRS, per-machine canonical config loading, injected controller/SCM_RIGHTS endpoints, bounded selector handling, and audit certainty are exercised with fakes. T035 remains open for an integrated installed cross-process executable using real kernel-owned controller/lease/guest listeners, config ownership lifecycle, continuous disconnect/deadline loop, and lifecycle/audit observation. Production completion also requires T039-T043 convergence on v2; v1 runtime wiring is forbidden. Authorized host proof remains under T022/T029/T031; code presence is not T022 proof)
