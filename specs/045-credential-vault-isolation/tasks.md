...
- [ ] T003 Run the authorized Ubuntu 24.04 managed-native hostile, grant/revoke, exhaustion, warm-start, cleanup, and end-to-end acceptance matrix and update only its evidence records in `specs/039-native-runtime-adoption/evidence/README.md` and `specs/039-native-runtime-adoption/evidence/` (BLOCKED: the 2026-08-29 authorized remote attempt in `evidence/live-proof-attempt-2026-08-29.md` observed Ubuntu 24.04/systemd 255 but failed seven managed-native prerequisites before provisioning; the shared host was not treated as disposable and no local/container substitute or hostile matrix was used)
...
- [ ] T022 [US3] Add fixed-verb service supervision and cleanup observation for the unprivileged credential broker without passing credential bytes to the root helper in `tools/native-helper/native-helper.py` and `sandbox/runtimes/managed/services.py` (BLOCKED: requires T032-T036, completed v2 convergence through T043, and authorized Ubuntu root-helper/service lifecycle proof; at the exact 2026-08-29 source revision all eight v2 helper lifecycle verbs still deliberately exit 69 as uninstalled, so the live attempt proved only fail-closed behavior and no local seam may be promoted as T022 completion)
...
- [ ] T029 Extend the authorized live native acceptance harness with exact binding, hostile no-leak, revoke, restart, exhaustion, cleanup, and timing checks in `tests/live_native_acceptance.py` (BLOCKED: requires T003, T022, T037, completed v2 convergence through T043, and a qualified disposable Ubuntu 24.04 host; the 2026-08-29 public fake-metadata action returned a bounded non-mutating refusal, while the current live harness still has no real v2 bind/request/revoke matrix, so offline harness tests and refusal evidence remain preparation only)
...
- [x] T035 [US3] Implement the reviewed unprivileged standalone broker executable and its instance-bound request/trusted-lease endpoints in `tools/native-helper/native-credential-broker.py`, then satisfy `tests/test_credential_broker_service_contract.py` without adding a default or adoptable runtime path. **LOCALLY COMPLETE; INDEPENDENTLY ACCEPTED BY SOL HIGH (LOCAL ONLY):** reciprocal derived configs pin the immutable guest registry, control-plane-derived private-veth tuple, canonical unit identities, fixed cgroup process authority, and compiler-derived reciprocal process-config digests; callers no longer provide own/peer config digests. Fixed no-follow role loaders validate both plans before one-PID start/observe/start pinning, and packet credentials only recheck the pinned tuple. Main constructs the fixed closed-first controller/guest/DNS/nft/typed-effect/service-loop graph and reverses partial construction; absent Linux/cgroup/nft/device authority remains a stable nonzero refusal before guest bytes or credential resolution. DNS uses a typed non-daemon process authority with an absolute monotonic deadline, maximum sixteen results, termination/join, and retained ownership on failed cleanup rather than an arbitrary blocking callback. Local seams cover immutable `GuestRequestV2` storage, the exact typed effect boundary, one controller-socket ACK reader, bounded guest/effect-worker ownership, reflection-free results, exact-set egress receipts, and sticky cleanup. Egress denial follows durable PRE -> refused/none POST -> 444-byte ACK -> SBR2 without effect entry. The terminal schedule uses overflow-checked absolute deadlines with a fixed 2,000-ms grace; connected `R+2000` ACK succeeds and `R+2001` refuses. This local completion remains closed and uninstalled; it is not T022/T029 Ubuntu 24.04 kernel/systemd proof or T031 human release review. No default/adoptable path exists, v1 has no runtime fallback, and support remains `implemented_unproven`/`adoptable=false` with `evidence_id=null`.
- [x] T036 [US3] Add secret-free digest-bound broker plans, fixed helper lifecycle verbs, broker-first cleanup observation/order, and inert dependency wiring in `sandbox/runtimes/managed/services.py`, `tools/native-helper/native-helper.py`, `sandbox/runtimes/managed/adapter.py`, and `sandbox/application/context.py`, with local coverage in `tests/test_managed_services.py`, `tests/test_native_cleanup_observation.py`, and `tests/test_credential_wiring.py`. **LOCALLY COMPLETE; INDEPENDENTLY ACCEPTED BY SOL HIGH (LOCAL ONLY):** reciprocal plans, the exact plan-bound fixed-verb executor, duplicate-safe bounded status parsing, broker-first exhaustive cleanup, and exact-type inert wiring exist. A fixed non-root controller executable and shared no-follow config/process runtime separate the controller role from the broker. The controller pins its own process, boundedly waits for the exact broker-owned listening seqpacket row, then pins the broker, creates one SO_PASSCRED-verified socket, performs exactly one T040 HELLO attempt, and stays persistently closed until signal or broker EOF when reviewed application authority/evidence is absent. Connect is never retried; cleanup is sticky, exhaustive, and exact-once. The broker remains the sole listener and guest/lease/effect owner. The helper still has exactly eight fixed v2 verbs and deliberately returns code 69 because unit/config installation and Ubuntu systemd/root-helper ownership proof belong to T022. No default/adoptable/v1 path or support/evidence promotion exists; T022/T029/T031 remain blocked, and support remains `implemented_unproven`, `adoptable=false`, with `evidence_id=null`.
