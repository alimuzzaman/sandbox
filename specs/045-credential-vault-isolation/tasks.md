...
- [ ] T035 [US3] Implement the reviewed unprivileged standalone broker executable and its instance-bound request/trusted-lease endpoints in `tools/native-helper/native-credential-broker.py`, then satisfy `tests/test_credential_broker_service_contract.py` without adding a default or adoptable runtime path (LOCAL ONLY: retained guest connection, authenticated controller endpoint, descriptor rendezvous, kernel-derived guest observation, controller-authorized typed execution through the existing broker gates, terminal SBRS delivery, bounded worker loop, strict config, and lifecycle identity are implemented under local fakes. The production `--serve` path remains deliberately unreachable because `live_transport_status()` returns `live_transport_unproven`; it is not a runnable local criterion. The lease frame's new `auth_form` field remains a v1 protocol addition needing independent review under T031, and authorized Ubuntu proof remains T022/T029. Code presence is not T022 proof)
