...
- [ ] T035 [US3] Implement the reviewed unprivileged standalone broker executable and its instance-bound request/trusted-lease endpoints in `tools/native-helper/native-credential-broker.py`, then satisfy `tests/test_credential_broker_service_contract.py` without adding a default or adoptable runtime path. **FOUNDATION ADDED; INTEGRATION STILL OPEN (LOCAL ONLY):** `credential_guest_protocol_v2.py` now owns the strict SBG2/SBR2 codec and complete immutable registry, default-deny RFC1918 private-veth projection and kernel-observation contract, canonical-digest full egress projection even when empty, one-resolution hostname/SNI/public-IPv4 decision shared with nft/effect execution, immutable authorized-effect context, phase-exact typed guest result, and tombstoned no-retry effect-entry contract. Reciprocal derived configs pin the same complete secret-free egress projection. This is production-shaped local code, but it is closed and uninstalled: T035 still requires the executable to own real controller/lease/guest listeners, config lifecycle, continuous disconnect/deadline handling, and lifecycle/audit cleanup observation. Completing that local implementation is distinct from T022/T029 Ubuntu 24.04 systemd/kernel proof and T031 human review. T035 stays incomplete; v1 runtime wiring is forbidden and support remains unpromoted.
