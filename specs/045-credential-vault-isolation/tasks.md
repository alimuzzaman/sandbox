...
- [ ] T035 [US3] Implement the reviewed unprivileged standalone broker executable and its instance-bound request/trusted-lease endpoints in `tools/native-helper/native-credential-broker.py`, then satisfy `tests/test_credential_broker_service_contract.py` without adding a default or adoptable runtime path (LOCAL ONLY: every local criterion is now implemented -- retained guest connection, authenticated controller endpoint, descriptor rendezvous, kernel-derived guest observation, controller-authorized typed execution through the existing broker gates, terminal SBRS delivery, bounded worker loop, strict config, lifecycle identity, and a closed-by-default runnable `--serve` path. Two items still gate completion: the lease frame's new `auth_form` field is a v1 protocol addition needing independent review under T031, and every test here uses unprivileged local fakes, so authorized Ubuntu proof remains T022/T029. Code presence is not T022 proof)
