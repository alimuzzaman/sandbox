...
export function welcome(): string {
  ...
  return `<div class="max-w-2xl mx-auto px-6 py-12">
    ...
    <div class="mt-7 grid grid-cols-3 gap-3">
      ${infoCard("Build", "Your agent implements features against a running install and verifies them live — not from memory.")}
      ${infoCard("Reproduce", "Hand your agent the environment and it reproduces the bug on a real stack — confirmed, not assumed.")}
    ...
    ${n
      ? `<p class="mt-6 text-[13px] text-neutral-400">${n} environment${n === 1 ? "" : "s"} ready — pick one on the left, or hand one to your agent.</p>`
