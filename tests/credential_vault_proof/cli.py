    ...
    try:
        ...
        if args.verb == "record-artifact":
            ...
            if not stat.S_ISREG(details.st_mode) or details.st_uid != os.getuid():
                _emit({"ok": False, "code": "artifact_unreadable", "location": path.name})
                return EXIT_REFUSED
            if details.st_size > bundle_module.MAX_ARTIFACT_BYTES:
                _emit({"ok": False, "code": "artifact_oversize", "location": path.name})
                return EXIT_REFUSED
