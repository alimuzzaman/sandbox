...
def _ready_observation(revision=None, services=("web",)):
    keys = ["LENZORA_SOURCE_REVISION"] if revision else []
    return {
        "schema_version": 1,
        "complete": True,
        ...
        "revision_checks": [
            {"service": service, "key": key, "observed": revision}
            for service in services for key in keys
        ],
        "phases": [{"phase": "observation", "state": "complete"}],
    }
...
class TestHostingManifest(unittest.TestCase):
    ...
    def test_git_root_boundary_ignores_hostile_repository_selectors(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            ...
            intended.mkdir()
            foreign.mkdir()
            subprocess.run(["git", "init", "-q"], cwd=intended, check=True)
            ...
            with patch.dict(os.environ, hostile, clear=False):
                resolved = hosting._git_root(intended)

        self.assertEqual(resolved, intended.resolve())
    ...
    def test_nested_manifest_declared_source_root_is_the_transfer_root(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            transfer_call = next(
                call for call in ssh.call_args_list
                if call.kwargs.get("input_data") is not None
            )
            command = transfer_call.args[1]
            self.assertIn("tar -xf - -C /srv/deploy/example-site", command)
            self.assertNotIn("/srv/deploy/example-site/site", command)
            archive = tarfile.open(fileobj=io.BytesIO(transfer_call.kwargs["input_data"]), mode="r:")
...
""",
    ...
    def test_initializer_status_command_accepts_env_prefix_and_rejects_stale_container(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            docker.write_text(
                ...
                " if 'config' in args: print(os.environ.get('HASH_OUTPUT', 'setup ' + 'a' * 64))\n"
    ...
    def test_apply_rejects_a_stale_running_service_revision(self, observe):
        with self._write(_manifest_with_derived_revision()) as directory:
            validated = hosting.validate_manifest(directory)
        observe.return_value = _ready_observation("b" * 40)
    ...
    @patch("sandbox.commands.hosting._observe_host_runtime")
    def test_apply_revision_probe_is_bundled_with_compose_observation(self, observe):
        with self._write(_manifest_with_derived_revision()) as directory:
            validated = hosting.validate_manifest(directory)
        revision = "a" * 40
        observe.return_value = _ready_observation(revision)
    ...
    def test_remote_health_retains_remote_retry_phases(self, ssh_run):
        ssh_run.return_value = subprocess.CompletedProcess([], 0, stdout=json.dumps({
            "schema_version": 1, "complete": True, "status": 200,
            "phases": [
                {"phase": "health", "state": "pending", "attempt": 1},
                {"phase": "health", "state": "ready", "attempt": 2},
            ],
        }), stderr="")
        runtime = {
            "loopback_port": 18001,
        ...
        receipt = hosting_cmd._verify_remote_health({}, runtime)

        self.assertEqual(ssh_run.call_count, 1)
        self.assertEqual(receipt["phases"][0]["state"], "pending")
    ...
    def test_host_status_reports_revision_and_bounded_service_health(self):
        ...
        observation = json.dumps({
            ...
            "revision_checks": [],
            "phases": [{"phase": "compose", "state": "complete"}],
        })
        with patch.object(hosting_cmd.remote, "resolve_sandbox_home", return_value="/srv/sandbox"), \
             patch.object(hosting_cmd, "_remote_checked", return_value=observation) as remote_checked:
        ...
        self.assertEqual(result["health"]["state"], "unverified")
        self.assertIn("unknown", result["health"]["reason"])
    ...
    def test_exact_observed_runtime_reconciles_a_stale_record_without_compose_reapply(self):
        revision = "7" * 40
        with self._write(_manifest_with_derived_revision()) as directory:
            validated = hosting.validate_manifest(directory)
        state = {"version": 1, "hosts": {
        ...
        self.assertEqual(record["runtime"]["topology"], {"state": "ready"})
        self.assertEqual(record["runtime"]["health"], {"state": "ready"})
        self.assertEqual(record["runtime"]["services"], observation["services"])
        saved_source = record["runtime"]["source_revision"]
        self.assertEqual(saved_source["state"], "ready")
        self.assertEqual(saved_source["checks"][0]["state"], "match")
        self.assertNotIn("observed", saved_source["checks"][0])
    ...
    def test_dirty_source_identity_change_cannot_choose_edge_only(self):
        ...
        self.assertEqual(hosting_cmd._runtime_apply_decision(
            ...
            config_digest="digest-1", source_state_identity="sha256:" + "2" * 64,
            source_state_clean=False, exact_runtime_proven=True,
        ), "refuse")
    ...
    def test_mismatched_revision_value_is_never_retained_or_exposed(self):
        with self._write(_manifest_with_derived_revision()) as directory:
            validated = hosting.validate_manifest(directory)
        hostile = "credential-looking-value-that-must-not-survive"
        ...
        self.assertEqual(classified["source_revision"]["checks"][0]["state"], "mismatch")
        self.assertNotIn(hostile, json.dumps(classified))
        self.assertNotIn("observed", classified["source_revision"]["checks"][0])
        key = hosting.state_key("myvps", validated)
        legacy = {"version": 1, "hosts": {key: {"runtime": {
            "state": "unverified", "source_revision": {
                "state": "degraded", "checks": [{
                    "service": "web", "key": "LENZORA_SOURCE_REVISION",
                    "state": "mismatch", "observed": hostile,
                }],
            },
        }}}}
        status = hosting_cmd._host_runtime_status(
            validated, {"provisioned": False}, "myvps", legacy,
        )
        self.assertNotIn(hostile, json.dumps(status))
    ...
    @patch("sandbox.commands.hosting.remote.ssh_run")
    def test_remote_health_uses_one_bounded_session_with_total_deadline(self, ssh_run, sleep):
        ssh_run.return_value = subprocess.CompletedProcess(
            [], 0,
            stdout=json.dumps({
                "schema_version": 1,
        ...
        self.assertTrue(receipt["complete"])
        self.assertEqual(ssh_run.call_count, 1)
        self.assertEqual(ssh_run.call_args.kwargs["timeout"], 65)
        sleep.assert_not_called()
    ...
    def test_host_status_includes_profiled_declared_services_without_counting_init_or_db(self):
        ...
        with self._write(manifest) as directory:
            validated = hosting.validate_manifest(directory)
        with patch.object(hosting_cmd.remote, "resolve_sandbox_home", return_value="/srv/sandbox"), \
             patch.object(hosting_cmd, "_observe_host_runtime", return_value={
                 **_ready_observation(services=("web", "worker", "db")),
                 "configured_services": ["web", "worker", "migrate", "db"],
             }) as observe:
            result = hosting_cmd._host_runtime_status(
    ...
    def test_host_status_marks_manifest_service_missing_from_compose_as_degraded(self):
        ...
        with patch.object(hosting_cmd.remote, "resolve_sandbox_home", return_value="/srv/sandbox"), \
             patch.object(hosting_cmd, "_observe_host_runtime", return_value={
                 **_ready_observation(), "configured_services": ["web"],
             }):
            result = hosting_cmd._host_runtime_status(
                validated, {"provisioned": True}, "myvps", {"version": 1, "hosts": {}},
            )
    ...
    def test_host_status_marks_compose_service_without_runtime_row_as_degraded(self):
        ...
        with patch.object(hosting_cmd.remote, "resolve_sandbox_home", return_value="/srv/sandbox"), \
             patch.object(hosting_cmd, "_observe_host_runtime", return_value={
                 **_ready_observation(), "configured_services": ["web", "worker"],
             }):
            result = hosting_cmd._host_runtime_status(
                validated, {"provisioned": True}, "myvps", {"version": 1, "hosts": {}},
            )
    ...
    def test_host_diagnose_combines_disk_images_and_source_revision_evidence(self):
        ...
        status = {
            ...
            "source_revision": {"state": "ready", "checks": [{
                "service": "web", "key": "LENZORA_SOURCE_REVISION",
                "provider": "pushed_commit_sha", "observed": revision,
                "expected": revision, "state": "match",
            }]},
        }
