...
OTHER_BOOT = "22222222-3333-4444-5555-666666666666"
...
class JobReconciliationTests(unittest.TestCase):
    ...
    def test_host_boot_change_is_interrupted_without_claiming_success(self):
        ...
        observed = ProcessIdentity(OTHER_BOOT, 101, "start", "nonce")
    ...
    def test_missing_child_identity_remains_running_without_loss_proof(self):
        row = self._running()
        self.repository.put_process_identity(row["job_id"], host_boot_id=BOOT, supervisor_pid=101,
    ...
    def test_on_read_orphaned_child_identity_is_interrupted(self):
        ...
        self.repository.put_process_identity(row["job_id"], host_boot_id=BOOT, supervisor_pid=101,
            supervisor_start_identity="supervisor", supervisor_nonce_hash="nonce", child_pid=202,
            child_pgid=202, child_start_identity="stale-child")
        ...
        with patch("sandbox.jobs.health.capture_process_identity", side_effect=capture), \
                patch("sandbox.jobs.health.os.kill", return_value=None), \
                patch("sandbox.jobs.health.process_absence_proven", return_value=True), \
                patch("sandbox.application.job_service.capture_process_identity", return_value=None), \
    ...
    def test_stale_heartbeat_and_terminal_rows_keep_truthful_lifecycles(self):
        ...
        state = self.service.get(stale["job_id"])
        self.assertEqual(state["lifecycle"], "running")
        self.assertIsNone(state["termination_reason"])
        self.assertEqual(state["health"], "supervisor_unresponsive")
        self.assertIsNone(state["health_evidence"]["supervisor_identity_valid"])
