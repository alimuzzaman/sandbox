...
class JobSchedulerTests(unittest.TestCase):
    ...
    def test_busy_submission_reports_advisory_position_and_blocker(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            first = service.submit(JobSubmission("test", temp, "p", "local", "same",
                ("echo", "one"), 60, source, request_id="first"))
            second = service.submit(JobSubmission("test", temp, "p", "local", "other",
                ("echo", "two"), 60, source, request_id="second"))
            queue = second["queue"]
            self.assertEqual(queue["position"], 1)
            self.assertEqual(queue["blocking_jobs"][0]["job_id"], first["job_id"])
            status = service.get(second["job_id"])
            self.assertEqual(status["queue"]["position"], 1)
            self.assertEqual(status["queue"]["blocking_jobs"][0]["job_id"], first["job_id"])
            repo.close()
    ...
    def test_new_admission_reaps_terminal_job_leases_without_status_poll(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            scheduler = JobScheduler(repo, max_parallel=1,
                                     min_free_memory_mb=0, min_free_disk_mb=0)
            first = repo.accept(JobSubmission("test", temp, "p", "local", "one",
                ("echo", "one"), 60, SourceIdentity("s"), request_id="first"))[0]
            scheduler.acquire(first)
            repo.transition(first["job_id"], "running")
            repo.transition(first["job_id"], "succeeded", exit_code=0)
            second = repo.accept(JobSubmission("test", temp, "p", "local", "two",
                ("echo", "two"), 60, SourceIdentity("s"), request_id="second"))[0]
            scheduler.acquire(second)
            self.assertEqual(len(scheduler.active()), 1)
            self.assertEqual(scheduler.active()[0]["job_id"], second["job_id"])
            repo.close()
