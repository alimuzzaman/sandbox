...
print("HERMES", int({
    ...
    'hermes_repo_sync', 'hermes_authorization_list', 'hermes_authorization_show',
...
class TestMcpServerSplit(unittest.TestCase):
    def test_tools_and_prompts_register(self):
        ...
        self.assertEqual(calls[11][0][1:3], ["authorization", "list"])
        self.assertEqual(calls[12][0][1:3], ["authorization", "show"])
        self.assertEqual(calls[13][0][1:3], ["authorization", "request"])
        self.assertIn("--job", calls[13][0])
        self.assertIn("--replay-origin", calls[13][0])
        self.assertEqual(calls[14][0][1:3], ["authorization", "approve"])
        self.assertIn("--confirm", calls[14][0])
        self.assertEqual(calls[15][0][1], "health")
        self.assertEqual(calls[16][0][1:3], ["worktree", "list"])
        self.assertEqual(calls[17][0][1:3], ["repo", "sync"])
        self.assertIn("--confirm", calls[17][0])
        self.assertIn("--confirm", calls[18][0])
        self.assertEqual(calls[19][0][1:3], ["cron", "catalog"])
        self.assertIn("--force-replace", calls[20][0])
        self.assertEqual(calls[21][0][1:3], ["cron", "verify"])
        self.assertEqual(calls[22][0][1:3], ["worktree", "inspect"])
        self.assertEqual(calls[23][0][1:3], ["worktree", "preserve"])
        self.assertIn("--confirm", calls[23][0])
