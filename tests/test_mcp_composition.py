...
class TestMcpComposition(unittest.TestCase):
    ...
    def test_builtin_group_manifest_is_exact_and_deterministic(self):
        ...
        expected = (
            ...
            "secrets", "plugin_check", "remote", "hermes", "recovery", "sync", "owned_storage", "delivery",
        )
    ...
    def test_runtime_scoped_catalogs_hide_irrelevant_tools(self):
        ...
        self.assertIn("feedback", wordpress)
        self.assertIn("feedback", compose)
    ...
    def test_builtin_groups_declare_a_compatibility_registration_boundary(self):
        ...
        self.assertEqual(
            {spec.group_id: spec.dependencies for spec in specs if spec.group_id in {"instances", "domains", "runtime", "jobs", "wp", "hermes", "resources", "feedback", "secrets", "sync", "owned_storage", "delivery"}},
            {
                "instances": (
        ...
        self.assertTrue(all(spec.dependencies == ("app",) for spec in specs
                            if spec.group_id not in {"instances", "domains", "runtime", "jobs", "wp", "hermes", "resources", "feedback", "secrets", "sync", "owned_storage", "delivery"}))
    ...
    def test_migrated_groups_do_not_wildcard_import_the_legacy_app_namespace(self):
        ...
        groups = (
            ...
            "abilities", "skills", "debug", "e2e", "ci", "asyncjobs",
            "plugin_check", "remote", "hermes", "recovery", "resources", "feedback",
        )
        for group in groups:
