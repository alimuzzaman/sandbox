...
class TestRecoveryScheduler(unittest.TestCase):
    def test_units_are_disabled_separate_and_use_lock_and_random_delay(self):
        ...
        self.assertIn("sb recovery create --scheduled --confirm --profile fixture", units["service"])
        self.assertIn("--scheduled", units["service"])
        self.assertIn("/usr/bin/env sb", units["service"])
