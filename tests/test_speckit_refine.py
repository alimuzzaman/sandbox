...
class SpeckitRefineArtifactTests(unittest.TestCase):
    def test_prd_template_is_first_class_and_phase_bounded(self):
        ...
        self.assertIn("**Drafting Configuration**:", template)
        self.assertIn("**Final Validation**: `PENDING` — independent readiness review", template)
        self.assertIn("independent readiness review verdict is `PASS`", template)
    ...
    def test_refine_skill_is_mirrored_and_forbids_downstream_artifacts(self):
        ...
        self.assertIn("active repository and user model/delegation policy", canonical)
        self.assertIn("## Final independent readiness validation", canonical)
        self.assertNotIn("gpt-5.6-", canonical)
        self.assertNotIn("Terra", canonical)
        self.assertNotIn("Sol", canonical)
    ...
    def test_implementation_model_routing_is_mirrored(self):
        ...
        for content in (canonical, mirrored):
            self.assertIn("active repository and user model/delegation policy", content)
            self.assertIn("Sandbox control-plane boundaries", content)
    ...
    def test_workflow_places_refinement_before_specification(self):
        ...
        self.assertIn("Material SDD Cycle", workflow)
        self.assertIn("change_class", workflow)
        self.assertIn("command: speckit.refine", workflow)
        self.assertIn("independent readiness review", workflow)
        self.assertNotIn("Terra", workflow)
        self.assertNotIn("Sol", workflow)
